...
import org.whispersystems.textsecuregcm.sqs.AMQPClient;
...
public class WhisperServerService extends Application<WhisperServerConfiguration> {
      ...
      throws Exception
  {
    ...
    //mq
    AMQPClient amqpClient = new AMQPClient(config.getAmqpConfiguration());
//    DirectoryQueue             directoryQueue             = new DirectoryQueue(config.getDirectoryConfiguration().getSqsConfiguration(),amqpClient);
    DirectoryQueue             directoryQueue             = new DirectoryQueue(amqpClient);
    ...
    JiGuangPush                jiGuangPush                = new JiGuangPush(config.getJpush());
    APNSender                  apnSender                  = new APNSender(accountsManager, config.getApnConfiguration(),jiGuangPush);
    GCMSender                  gcmSender                  = new GCMSender(accountsManager, config.getGcmConfiguration().getApiKey(),jiGuangPush);
...
//    TwilioSmsSender          twilioSmsSender     = new TwilioSmsSender(config.getTwilioConfiguration(),config.getAliyun());
    TwilioSmsSender          twilioSmsSender     = new TwilioSmsSender(config.getAliyun());
