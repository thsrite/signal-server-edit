...
//  public DirectoryQueue(SqsConfiguration sqsConfig,AMQPClient amqpClient) {
  public DirectoryQueue(AMQPClient amqpClient) {
...
//      sqs.sendMessage(sendMessageRequest);
      ...
      msgMap.put("id",number);
      msgMap.put("uuid",uuid.toString());
      msgMap.put("action",action);
      amqpClient.sendMessage(GsonUtil.getJson(msgMap));
      logger.info("amqp send message success! ==>" + GsonUtil.getJson(msgMap));
    } catch (AmazonServiceException ex) {
      serviceErrorMeter.mark();
      logger.warn("amqp service error: ", ex);
    } catch (AmazonClientException ex) {
      clientErrorMeter.mark();
      logger.warn("amqp client error: ", ex);
    } catch (Throwable t) {
      logger.warn("amqp unexpected error: ", t);
    }
  }
